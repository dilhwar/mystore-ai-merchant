            ...
            >
              <VStack space="sm">
                ...
                <Box
                  ...
                  borderRadius="$xl"
                  bg={isDark ? stat.darkBgColor : stat.bgColor}
                  alignItems="center"
                  justifyContent="center"
                >
                  <Icon size={sizeStyles.iconSize} color={colors[stat.color.replace('$', '') as keyof typeof colors]} strokeWidth={2} />
                </Box>

                {/* Value */}
                <Heading size={sizeStyles.valueSize} color="$textLight" $dark-color="$textDark" numberOfLines={1}>
                  {stat.value}
                </Heading>

                {/* Title & Trend */}
                <HStack justifyContent="space-between" alignItems="center">
                  <Text fontSize={sizeStyles.titleSize} color="$textSecondaryLight" $dark-color="$textSecondaryDark" fontWeight="$medium" numberOfLines={1} flex={1}>
                    {stat.title}
                  </Text>
